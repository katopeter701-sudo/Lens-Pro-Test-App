...
if role == "Admin Dashboard":
    ...
    with t1:
        with st.form("manual"):
            p, t = st.text_input("Course Name"), st.text_input("Module Topic")
            w, y = st.number_input("Week", 1, 15), st.text_input("YouTube/Slide Link")
    ...
    if search:
        ...
        for item in res.data:
            with st.expander(f"Week {item['week']} - {item['course_name']}"):
                ...
                st.link_button("Notes", item['notes_url'])
